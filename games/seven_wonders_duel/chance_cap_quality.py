...
def measure_edge(state, index, specs, evaluator, offsets_list, search_seed):
    """One edge: exact Q over the full outcome space vs each capped support."""
...
def summarise_level_a(rows, offsets_list) -> dict:
    ...
    for offsets in offsets_list:
        ...
        summary[offsets] = {
            ...
            # Fraction of edges whose retained support still contains the
            # single worst outcome for the player to move.
...
def summarise_level_b(comparisons) -> dict:
    ...
    for label, rows in comparisons.items():
        ...
        out[label] = {
            ...
            "policy_kl_mean": mean(r["policy_kl"] for r in rows),
            "policy_kl_p95": percentile([r["policy_kl"] for r in rows], 0.95),
...
def main(argv=None) -> int:
    ...
    rows, py_states, rust_games, skipped = [], [], [], 0
    for seed, first_player, prefix in iter_roots(
        ...
        rows.extend(
            measure_edge(state, index, specs, evaluator, args.offsets, search_seed)
    ...
    if not args.skip_search and rust_games:
        ...
        seeds = [args.search_seed] * len(rust_games)
        ...
        control = run_searches(
            adapter,
            rust_games,
            [args.control_seed] * len(rust_games),
            args.sims,
            args.top_k,
            0,
            args.batch_cap,
        ...
        for offsets in args.offsets:
            ...
            comparisons[f"offsets_{offsets}"] = [
                compare_search(a, b) for a, b in zip(baseline, capped)
            ]
            forced_rows[str(offsets)] = sum(
                result["nn_work"]["forced_rows"] for result in capped
            )
        forced_rows["0"] = sum(
            result["nn_work"]["forced_rows"] for result in baseline
        )
        report["forced_rows"] = forced_rows

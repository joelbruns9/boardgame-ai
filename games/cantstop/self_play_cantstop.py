...
class TeeLogger:
    """
    Mirror stdout/stderr to multiple streams (console + log file).
    Flushes immediately so long overnight runs preserve logs even if
    interrupted.
    """
...
from games.cantstop.symmetry import augment_records
...
def play_mcts_game(mcts, num_simulations=20, global_temp_mult=1.0,
    ...
    # Drop records from games that didn't terminate. state.winner is None
    # when the game hit max_turns without a winner. The labelling block
    # below would otherwise compute `1.0 if player == None else 0.0` for
    # every record — labelling both players as losers, which silently
    # poisons the value training signal. Dropping is safer than blending
    # with 0.5: timed-out games are usually pathological (early-training
    # network plays poorly and games drag), not representative.
...
class _BatchedGameRunner:
    ...
    def labeled_records(self):
        ...
        # Same defense as play_mcts_game — don't label timeout records.
        # When the runner hit max_turns without state.game_over, winner
        # is None and naive labelling would call every record a loss.
...
def train_on_buffer(model, replay_buffer, device,
    ...
    print(
        f"  Training sample: {len(records):,} records "
        f"from replay buffer size {replay_buffer.size():,} "
    ...
    # ---- Mirror augmentation: training split only (val stays a clean measure) ----
    if augment:
        tr_records = augment_records(tr_records)
...
# ---- ATOMIC, ROTATING CHECKPOINT WRITE ----

def _atomic_save_with_rotation(checkpoint, dest_path):
    """
    Save `checkpoint` to `dest_path` atomically, rotating the existing
    file (if any) to `dest_path + '.bak'`.

    Why:
      torch.save is NOT atomic. A crash mid-write leaves dest_path
      truncated/corrupt. If dest_path is best_model.pt, the strongest
      model is irrecoverably lost.

    Procedure:
      1. Write to dest_path + '.tmp' (full write, fully flushed).
      2. Rotate existing dest_path -> dest_path + '.bak' (best-effort).
      3. os.replace(.tmp, dest_path) — atomic on POSIX and Windows.

    Result:
      - On clean success: dest_path holds the new checkpoint;
        dest_path.bak holds the previous one (or absent if first write).
      - On crash during step 1: dest_path unchanged; .tmp may exist as
        garbage but is harmless on next run (will be overwritten).
      - On crash between step 2 and step 3: dest_path is missing but
        .bak is intact. Recover by `mv dest_path.bak dest_path`.
    """
    ...
    if os.path.exists(dest_path):
        try:
            os.replace(dest_path, bak_path)
        except OSError as e:
            # Non-fatal: log and continue. We'd rather have an updated
            # best_model.pt without a .bak than not update at all.
            print(f"  [warning] could not rotate {dest_path} -> {bak_path}: {e}",
                  flush=True)

    os.replace(tmp_path, dest_path)


def _should_overwrite_best(new_model, best_model_path, eval_games,
                            eval_sims, num_workers, output_dir,
                            best_overwrite_threshold):
    """
    Return (is_new_best, win_rate) — whether `new_model` should
    overwrite the on-disk best model.

    Runs a head-to-head eval (new vs current best on disk) and applies a
    stricter threshold than the accept_floor. Returns the measured
    win-rate alongside the decision so callers can log it.
    """
    wr = evaluate_networks(
        new_model,
    ...
    return (wr >= best_overwrite_threshold), wr
...
def self_play_loop(
    ...
    Two-tier checkpointing:
      Tier 1 (accept_floor, default 0.50): governs whether to advance
        the training chain. Accepted models become the next iteration's
        starting point and are saved as model_iter_NNN_accepted.pt.
      Tier 2 (best_overwrite_threshold, default 0.55): governs whether
        to overwrite best_model.pt. A separate eval is run pitting the
        newly-accepted model against the CURRENT best_model.pt on disk.
        Only a clear win (default 55%) updates best_model.pt. This
        protects the global-best checkpoint from being lost to noise,
        local drift, or errored runs. Writes are atomic with .bak
        rotation, so a crash mid-write can always be recovered.
    """
    if device is None:
    ...
    # Fix C is the default/safe training path. When enabled, it owns
    # batching across independent sync searches and must not also run
    # within-tree async.
    if batch_sync_searches and batch_sync_searches > 1:
        target_inflight = 1
        warmup_sims = 0

    # ---- Tee-style logging (console + file) ----
    if log_file is None:
    ...
    sys.stdout = TeeLogger(sys.__stdout__, _log_handle)
    sys.stderr = TeeLogger(sys.__stderr__, _log_handle)
    ...
    print(f"  Inflight:      {target_inflight} (async disabled)")
    print(f"  Warmup:        {warmup_sims} sequential sims/search")
    print(f"  Games/worker:  {games_per_worker} concurrent threads")
    if batch_sync_searches and batch_sync_searches > 1:
        print(f"  Batch-sync:    {batch_sync_searches} active sync searches per thread")
    ...
    # Stable "best" checkpoint path. NOT overwritten on every acceptance —
    # only when the new model clearly beats the on-disk best (see
    # best_overwrite_threshold). If best_model.pt does not yet exist (first
    # run), it is seeded from the initial model so the comparison gate
    # always has something to compare against.
    best_model_path = os.path.join(output_dir, 'best_model.pt')

    if not os.path.exists(best_model_path):
        # Seed with placeholder numeric values so downstream consumers
        # (load_model, etc.) can format these fields. The seeded model
        # has never been evaluated, so we use a neutral 0.5 win_rate and
        # the initial model's val_loss if known (else 0.0). These values
        # are just placeholders — they are never used for any decision,
        # only displayed when load_model logs metadata.
        _atomic_save_with_rotation(
            {'model_state': current_model.state_dict(),
             'iteration': 0,
             'win_rate': 0.5,
             'val_loss': 0.0,
             'temp_mult': 1.0,
             'seeded_from': initial_model_path},
            best_model_path,
        )
        print(f"  Seeded best_model.pt from initial model "
    ...
    try:
        for iteration in range(1, iterations + 1):
            ...
            if win_rate >= accept_floor:
                ...
                _atomic_save_with_rotation(checkpoint, save_path)

                current_model_path = save_path  # workers use new model
                accepted += 1
                print(f"  Saved iteration checkpoint: {save_path}")

                # ---- Tier-2 gate: is this actually a new global best? ----
                # Run a SEPARATE eval pitting the new model against the
                # current on-disk best. Acceptance against the previous
                # accepted model does NOT imply being better than the
                # all-time best — drift, noise, or rock-paper-scissors
                # cycles can produce a worse model that still passes the
                # 50% floor against the immediately-previous one.
                # ---- Tier-2 gate: periodic global-best eval ----
                if iteration % 5 == 0:
                    is_new_best, wr_vs_best = _should_overwrite_best(
                        new_model=new_model,
                        ...
                        num_workers=num_workers,
                        output_dir=output_dir,
                        best_overwrite_threshold=best_overwrite_threshold,
                    )

                    if is_new_best:
                        _atomic_save_with_rotation(checkpoint, best_model_path)
                        print(f"  ★ NEW BEST ({wr_vs_best:.1%} vs prior best, "
                            f"threshold {best_overwrite_threshold:.0%}) — "
                            f"updated {best_model_path}")
                    else:
                        print(f"  Not a new best ({wr_vs_best:.1%} vs prior best, "
                            f"need >={best_overwrite_threshold:.0%}). "
                            f"Keeping existing {best_model_path}.")

                    history_extra_wr_vs_best = float(wr_vs_best)
                    history_extra_is_new_best = bool(is_new_best)

                else:
                    print(f"  Skipping best-model eval this iteration.")
                    history_extra_wr_vs_best = None
                    history_extra_is_new_best = False
            ...
            history.append({
                'iteration':   iteration,
                'win_rate':    float(win_rate),
                'wr_vs_best':  history_extra_wr_vs_best,
                'is_new_best': history_extra_is_new_best,
                'val_loss':    float(val_loss),
                'temp_mult':   float(temp_mult),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inflight",   type=int,   default=1,
                        help="Concurrent in-flight MCTS sims per "
                             ...
                             "1. Higher values give bigger GPU "
    ...
    parser.add_argument("--warmup",     type=int,   default=0,
                        help="Sequential warmup sims at the start of "
                             "each MCTS search. Default 0. Required "
    ...
    parser.add_argument("--games-per-worker", type=int, default=2,
                        ...
                        help="Concurrent games per worker process "
                             ...
                             "threaded behavior.")
    parser.add_argument("--batch-sync-searches", type=int, default=8,
                        ...
                        help="Fix C experimental path: number of active "
                             ...
                             "to disable.")
    parser.add_argument("--best-overwrite-threshold", type=float, default=0.55,
                        dest="best_overwrite_threshold",
                        help="Win rate vs CURRENT best_model.pt required to "
                             "overwrite it. Independent of --floor (which "
                             "controls chain advancement). Default 0.55 — "
                             "high enough that eval noise alone won't "
                             "trigger overwrites.")
    parser.add_argument("--learning-rate", "--lr", type=float, default=3e-5,
                        dest="learning_rate",
                        help="Adam optimizer learning rate for the training "
                             "stage. Default 3e-5 is fine-tuning-style; "
                             "consider 1e-4 to 3e-4 if train loss is flat "
                             "across epochs (a strong sign the optimizer "
                             "isn't moving meaningfully).")
    parser.add_argument("--log-file", type=str, default=None,
                        dest="log_file",
                        help="Optional path for tee-style run logging. "
                             "Default: output_dir/run_YYYYMMDD_HHMMSS.log")
    parser.add_argument("--no-augment", action="store_true", dest="no_augment",
                        help="Disable mirror augmentation (for an A/B baseline).")
    ...
    self_play_loop(
        ...
        best_overwrite_threshold=args.best_overwrite_threshold,

...
5. At least 70 move-budget units remain for a complete panel.
...
explicit. It also means a 100-simulation move can normally afford one exhaustive
panel plus 30 real paths, but not a second panel. That is intentional selective
refinement: PUCT determines which action gets the expensive exact reveal mean.
...
- Focused Rust regression
  ...
  exact panel mean, and probability-aware replacement after a real backup.
- Python syntax compilation: passed.
- Python production-boundary regression: two tests passed. A two-slot mock
  evaluator drives complete games through the real `step()`/`update()` API and
  checks panel accounting plus the unchanged 12-field replay tuples; a second
  test checks invalid closed-loop configuration is rejected.
...
- Strength and throughput should next be measured in a matched self-play A/B.

...
def run_self_play_training(cfg: SelfPlayConfig, verbose: bool = True) -> dict:
    ...
    selfplay_net = net
    selfplay_source = "learner_latest"
    if cfg.gated_selfplay:
        if not cfg.current_best_path or not os.path.exists(cfg.current_best_path):
            raise FileNotFoundError(
                f"--gated_selfplay requires --current_best_path to exist: "
                f"{cfg.current_best_path!r}"
            )
        selfplay_net = load_generator_net(cfg.current_best_path, cfg)
        selfplay_source = str(cfg.current_best_path)
        if verbose:
            print(f"Gated self-play generator: {cfg.current_best_path}")
    ...
    try:
        for it in range(1, cfg.n_iterations + 1):
            ...
            if (cfg.gated_selfplay and iter_cfg.promotion_every
                    and it % int(iter_cfg.promotion_every) == 0):
                if verbose:
                    print(f"  promotion: evaluating learner vs self-play generator "
                ...
                if iter_cfg.promotion_skip_fixed_suite:
                    ...
                    base_fixed = fixed_suite_summary_for_net(
                        ...
                        suite=iter_cfg.promotion_fixed_suite,
                        device=iter_cfg.device,
                        checkpoint_label=f"selfplay_generator_iter_{it:04d}",
                    )
                    fixed_cmp = compare_fixed_suite(
                ...
                if decision.passed:
                    ...
                    if iter_cfg.promotion_update_best:
                        if checkpoint_path is None:
                            print("WARNING: promotion_update_best requested but no "
                            ...
                            payload = promotion_payload(
                                ...
                                extra={
                                    "source": "self_play.gated_selfplay",
                                    "iteration": it,
...
if __name__ == "__main__":
    ...
    p.add_argument("--promotion_games", type=int, default=400,
                   help="total head-to-head games for in-run promotion checks")
    p.add_argument("--promotion_sims", type=int, default=400,

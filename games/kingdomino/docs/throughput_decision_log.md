...
Status: rejected for current 48ch/6b laptop inference path
Priority: do not use by default

Hypothesis:
- CUDA convolutions may run faster when board tensors and model weights use
  `channels_last` memory format.

Result (2026-06-29, laptop CUDA, 48ch/6b, TF32, cudnn_benchmark, batch list
32/64/128/137/192/256):
- NCHW baseline: batch 137 = 9.05 ms, 66.1 us/sample; peak 15.6k evals/s.
- channels_last: batch 137 = 9.92 ms, 72.4 us/sample; peak 14.1k evals/s.
- channels_last was about 10% slower at the live-ish batch size.
- Profiler explanation: conv kernel time dropped somewhat, but channels_last
  introduced much more `contiguous` / `clone` / `copy_` layout work and made
  GroupNorm/layout conversion cost larger. Net result was slower.

Decision:
- Do not wire channels-last into `self_play.py` for this model/hardware path.

Reopen if:
- The norm changes away from GroupNorm, the network architecture changes, AMP is
  revisited on different hardware, or the cloud 5090 forward benchmark shows a
  different layout profile.

## BatchNorm Instead Of GroupNorm

Status: candidate; speed win in inference-only benchmark, needs training-quality A/B
Priority: medium-high if throughput remains forward-bound

Hypothesis:
- GroupNorm is a meaningful inference cost. BatchNorm in eval mode can use stored
  running statistics and cheaper cuDNN kernels, making self-play inference faster.

Result (2026-06-29, laptop CUDA, 48ch/6b, TF32, cudnn_benchmark, batch list
32/64/128/137/192/256):
- GroupNorm baseline: batch 137 = 8.96 ms, 65.4 us/sample; peak 15.5k evals/s.
- BatchNorm eval-mode: batch 137 = 7.92 ms, 57.8 us/sample; peak 17.3k evals/s.
- BatchNorm was about 12% faster at the live-ish batch size.
- Profiler explanation: GroupNorm's ~16% CUDA block is replaced by cuDNN
  BatchNorm at roughly half that cost; convs remain the dominant cost.

Decision:
- Do not change production training by speed result alone. This is a model
  quality/callibration decision because BatchNorm relies on running stats learned
  from replay-buffer minibatches.
- Decide this **before starting a training run**. Switching GroupNorm to
  BatchNorm in the middle of training is not valid: checkpoints are architecturally
  different, BatchNorm needs running statistics accumulated under that training
  regime, and self-play targets would be generated by a different evaluator.
- Worth a short training-quality A/B if a 10-12% inference gain is valuable:
  compare loss curves, policy entropy, value calibration/Brier, and Elo/self-play.

Reopen if:
- Cloud 5090 remains forward-bound after the core training loop is stable, or the
  training batch distribution makes BatchNorm running stats look reliable.
